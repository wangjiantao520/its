import { NextRequest, NextResponse } from 'next/server';
import pool, { initDatabase } from '@/lib/db';

// GET /api/intelligent-project-quotas - 获取智能化项目定额列表
export async function GET(request: NextRequest) {
  try {
    await initDatabase();

    const searchParams = request.nextUrl.searchParams;
    const keyword = searchParams.get('keyword') || '';
    const category = searchParams.get('category') || '';
    const page = Math.max(1, parseInt(searchParams.get('page') || '1', 10));
    const limit = Math.max(1, Math.min(200, parseInt(searchParams.get('limit') || '20', 10)));
    const offset = (page - 1) * limit;

    let whereClause = '';
    const params: any[] = [];

    if (keyword) {
      whereClause += ' WHERE (name LIKE ? OR id LIKE ? OR remark LIKE ? OR brand_model LIKE ? OR description LIKE ?)';
      params.push(`%${keyword}%`, `%${keyword}%`, `%${keyword}%`, `%${keyword}%`, `%${keyword}%`);
    }

    if (category) {
      if (whereClause) {
        whereClause += ' AND category = ?';
      } else {
        whereClause += ' WHERE category = ?';
      }
      params.push(category);
    }

    // 获取总数
    const [countResult] = await pool.execute(
      `SELECT COUNT(*) as total FROM intelligent_project_quotas${whereClause}`,
      params
    );
    const total = (countResult as any[])[0].total;

    // 分页查询（LIMIT/OFFSET 不支持预编译占位符，直接拼接整数）
    const [rows] = await pool.execute(
      `SELECT * FROM intelligent_project_quotas${whereClause} ORDER BY sort_order ASC, serial_number ASC LIMIT ${limit} OFFSET ${offset}`,
      params
    );

    const totalPages = Math.ceil(total / limit);

    return NextResponse.json({
      success: true,
      data: rows,
      pagination: {
        page,
        limit,
        total,
        totalPages,
      },
    });
  } catch (error) {
    console.error('获取智能化项目定额列表失败:', error);
    return NextResponse.json(
      { success: false, error: '获取智能化项目定额列表失败' },
      { status: 500 }
    );
  }
}

// POST /api/intelligent-project-quotas - 新增智能化项目定额
export async function POST(request: NextRequest) {
  try {
    await initDatabase();

    const body = await request.json();
    const { id, serialNumber, category, name, brandModel, description, deductibleTaxRate, unit, price, remark, sortOrder } = body;

    if (!id || !category || !name || !unit || price === undefined) {
      return NextResponse.json(
        { success: false, error: '缺少必填字段（编号、分类、名称、单位、单价）' },
        { status: 400 }
      );
    }

    // 检查ID是否重复
    const [existing] = await pool.execute(
      'SELECT id FROM intelligent_project_quotas WHERE id = ?',
      [id]
    );
    if ((existing as any[]).length > 0) {
      return NextResponse.json(
        { success: false, error: '定额编号已存在' },
        { status: 400 }
      );
    }

    await pool.execute(
      `INSERT INTO intelligent_project_quotas (id, serial_number, category, name, brand_model, description, deductible_tax_rate, unit, price, remark, sort_order)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, serialNumber || 0, category, name, brandModel || '', description || '', deductibleTaxRate || 0, unit, price, remark || '', sortOrder || 0]
    );

    return NextResponse.json({
      success: true,
      data: { id },
    });
  } catch (error) {
    console.error('新增智能化项目定额失败:', error);
    return NextResponse.json(
      { success: false, error: '新增智能化项目定额失败' },
      { status: 500 }
    );
  }
}

// PUT /api/intelligent-project-quotas - 编辑智能化项目定额
export async function PUT(request: NextRequest) {
  try {
    await initDatabase();

    const body = await request.json();
    const { id, serialNumber, category, name, brandModel, description, deductibleTaxRate, unit, price, remark, sortOrder } = body;

    if (!id) {
      return NextResponse.json(
        { success: false, error: '缺少定额编号' },
        { status: 400 }
      );
    }

    // 检查是否存在
    const [existing] = await pool.execute(
      'SELECT id FROM intelligent_project_quotas WHERE id = ?',
      [id]
    );
    if ((existing as any[]).length === 0) {
      return NextResponse.json(
        { success: false, error: '定额项不存在' },
        { status: 404 }
      );
    }

    await pool.execute(
      `UPDATE intelligent_project_quotas
       SET serial_number = ?, category = ?, name = ?, brand_model = ?, description = ?, deductible_tax_rate = ?, unit = ?, price = ?, remark = ?, sort_order = ?
       WHERE id = ?`,
      [serialNumber || 0, category, name, brandModel || '', description || '', deductibleTaxRate || 0, unit, price, remark || '', sortOrder || 0, id]
    );

    return NextResponse.json({
      success: true,
      data: { id },
    });
  } catch (error) {
    console.error('编辑智能化项目定额失败:', error);
    return NextResponse.json(
      { success: false, error: '编辑智能化项目定额失败' },
      { status: 500 }
    );
  }
}

// DELETE /api/intelligent-project-quotas - 删除智能化项目定额
export async function DELETE(request: NextRequest) {
  try {
    await initDatabase();

    const body = await request.json();
    const { id } = body;

    if (!id) {
      return NextResponse.json(
        { success: false, error: '缺少定额编号' },
        { status: 400 }
      );
    }

    const [result] = await pool.execute(
      'DELETE FROM intelligent_project_quotas WHERE id = ?',
      [id]
    );

    if ((result as any).affectedRows === 0) {
      return NextResponse.json(
        { success: false, error: '定额项不存在' },
        { status: 404 }
      );
    }

    return NextResponse.json({
      success: true,
      message: '删除成功',
    });
  } catch (error) {
    console.error('删除智能化项目定额失败:', error);
    return NextResponse.json(
      { success: false, error: '删除智能化项目定额失败' },
      { status: 500 }
    );
  }
}

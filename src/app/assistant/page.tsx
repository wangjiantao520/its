...
function AssistantContent() {
  ...
  const loadSession = useCallback(async (sessionId: string) => {
    try {
      ...
      if (result.success && result.data) {
        ...
        try { localStorage.setItem("its_active_assistant_session", sessionId); } catch { /* ignore */ }
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      ...
      // 切换页面/重新打开时恢复上次的会话（localStorage 跨会话保留）
      let lastSessionId: string | null = null;
      try { lastSessionId = localStorage.getItem("its_active_assistant_session"); } catch { /* ignore */ }
      if (lastSessionId) {
    ...
    try {
      ...
      const parser = createAssistantStreamParser((event) => {
        if (event.type === "start") {
          ...
          try { localStorage.setItem("its_active_assistant_session", event.session_id); } catch { /* ignore */ }
          void loadSessions();
  ...
  const handleNewChat = () => {
    ...
    try { localStorage.removeItem("its_active_assistant_session"); } catch { /* ignore */ }
  };

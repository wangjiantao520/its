...
try {
  ...
  for (let i = 0; i &lt; dataRows.length; i++) {
    ...
    const nameIndex = headers.findIndex(h =&gt; h &amp;&amp; (String(h).includes('名称') || String(h).includes('name')));
...
export const FULL_DEVICE_QUOTAS = ${JSON.stringify(devices, null, 2)};
...
export function getDevicesByLevel(level) {
...
export function searchDevices(keyword) {
  ...
  return FULL_DEVICE_QUOTAS.filter(d =&gt; 
    (d.name &amp;&amp; d.name.toLowerCase().includes(lowerKeyword)) ||
    (d.model &amp;&amp; d.model.toLowerCase().includes(lowerKeyword)) ||
    (d.category &amp;&amp; d.category.toLowerCase().includes(lowerKeyword))
  );
}
...
function getEngineerLevel(level) {
  const levels = {
    'A': '初级',
    'B': '初级',
    'C': '中级',
...
function getBasePriceByLevel(level) {
  const prices = {
    'A': 200,
    'B': 180,
    'C': 300,

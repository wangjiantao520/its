...
test('returns 401 when no valid session exists', async () => {
  const result = await authorizeRequest(request, undefined, async () => null);
...
test('returns 403 when the session role is not allowed', async () => {
  ...
  const result = await authorizeRequest(request, ['admin'], async () => member);
  assert.equal(result.ok, false);
  if (result.ok) return;
...
test('returns the session when its role is allowed', async () => {
  const admin: ApiSession = { role: 'admin' };
  const result = await authorizeRequest(request, ['admin'], async () => admin);
